...
def test_all_twenty_two_generated_allocation_keys_are_audited_once() -> None:
    ...
    assert len(certificate.audited_incidence_dimension_bounds) == 22
    assert {
        key for key, _bound in certificate.audited_incidence_dimension_bounds
...
def test_no_true_split_rank_stratum_can_support_a_threefold() -> None:
    ...
    assert certificate.maximum_split_incidence_dimension == max(
        bound for _key, bound in certificate.audited_incidence_dimension_bounds
    )
    assert set(
        bound for _key, bound in certificate.audited_incidence_dimension_bounds
    ) == {2}
