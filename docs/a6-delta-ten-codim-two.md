...
| expected codimension three, dimension two | 14 | dense clean nonsplit surfaces for `C4+6N` and `C2+C3+5N` are audited; the `C4` residual is bounded below surface dimension, and the `C2+C3` coefficient-rank-drop curve is exactly inconsistent; omitted pair charts and non-clean equisingular loci remain; see the codimension-three note |
...
5. treat the remaining twelve expected codimension-three profiles and the
   omitted boundaries of `C4+6N` and `C2+C3+5N`; and
