...
At the stage recorded in this note, the genus and large-link calculation left
one candidate at collision delta seven: affine degrees `(3,10)`, projective
infinity pair `(7,10)`.  This note proves that the **nondegenerate open
stratum** of its complete four-parameter normal-form family has cyclic affine
complement.  Hence that open stratum cannot support the required connected
`A6` monodromy with single-3-cycle meridians.
...
This is a conditional, computer-assisted generic-stratum exclusion.  This
note by itself does not classify the collision-degeneration walls, derive the
four hypotheses for arbitrary Keller branches, eliminate the unrestricted
`A6` passport, or prove the plane Jacobian conjecture.  The later
[delta-seven wall audit](a6-delta-seven-walls.md) supplies the missing wall
classification and supersedes this note's historical frontier: the combined
conditional bound is `Delta>=10`, and equality at the coarse link stage leaves
only affine degrees `(4,9)` with projective infinity pair `(5,9)`.
...
## 5. Propagation and the historical frontier
...
The follow-up [delta-seven wall audit](a6-delta-seven-walls.md) excludes the
full `T` wall, every positive-dimensional `G` stratum, and all finite
endpoints under the same standing hypotheses.  Its subsequent exact link
scan excludes collision deltas eight and nine.  The current combined
conditional conclusion is therefore

\[
  \boxed{\Delta\ge10,\qquad
  \Delta=10\Longrightarrow(a,d)=(4,9),\quad(d-a,d)=(5,9)}
\]

at the coarse link stage.  This does not construct the equality candidate.
...
The follow-up's checked Sage sources regenerate the generic-open presentation
above and the three excess-three presentations only.  The component checker
`tools/check_a6_delta_seven_components.sage` independently verifies the exact
positive-dimensional incidence algebra.  The triple-wall, excess-one/two,
and finite-endpoint relation words are replayed exactly by the dependency-free
checkers, but their original Zariski--van Kamp extraction remains trusted and
is not regenerated by a checked script in this repository.  See the
[wall audit reproduction section](a6-delta-seven-walls.md#9-reproduction) for
